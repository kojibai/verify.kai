...
/* ──────────────────────────
   ...
   kairosEpochNow() is bigint → this module’s stamps are number.
   Epoch ms (~1.7e12) is safely representable as number.
────────────────────────── */
const nowMsNumber = (): number => {
...
export function writeDebitsStored(
  ...
  if (!h) return;
  try {
    localStorage.setItem(debitsKey(h, token ?? null), encodeDebitsQS(d));
  } catch {
    // noop
  }
}
...
export function mergeDebitQS(a: DebitQS | null, b: DebitQS | null): DebitQS {
  ...
  out.originalAmount = typeof aAmt === "number" ? aAmt : typeof bAmt === "number" ? bAmt : undefined;
